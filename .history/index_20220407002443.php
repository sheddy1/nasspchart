...
<body>
    ...
    <?php
        ...
        else if($page == 2)
        {
            ...
            if($edit_chart == 2)
            {
                echo"<div class='view-charts-scrool'>";
                $user = "root";
                $pass = "";
                $db ="nassp";
                $db = new mysqli('localhost',$user,$pass,$db)
                ...
                $get_name="SELECT * FROM employees WHERE table1";

                    $run_get_name = mysqli_query($db,$get_name);
                    
                    $row = mysqli_fetch_assoc($run_get_name);

                    $name_table = $row['table1'];

                for($x = 0; $x<6; $x++)
                {
                    // echo"<label class='slidephotos'> $name_table</label>";
                    echo"<table>";
                    echo"<td class='device1-label'>".$row['name_table']."</td>";\
                    echo"</table>";
                }
