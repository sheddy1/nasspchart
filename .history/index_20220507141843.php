...
<body>
    <div id="header">
        ...
        <form action="" method="POST">
            <button class="btn  view-chart" name="view-chart">
                View Chart
            </button>

            <button class="btn  create-chart" name="create-chart">
                Create Chart
            </button>
        </form>
    </div>
    ...
    <?php
        $_SESSION['page'] = 2;
        $_SESSION['edit_chart'] = 2;

        // $page = $_SESSION['page'];
        // $edit_chart = $_SESSION['edit_chart'];
         ...
         if($_SESSION['page']== 1)
        {
         ...
         <input type='checkbox' value=1 name='all_employees' class='all_employees'>
         <label class='add_employees_label'>Add all employees</label>
        ...
        if(isset($_POST['btn-chart']))
        {
            ...
            echo"<script>alert('Chart has been created');</script>";

            $name = $_POST['chart-name-text'];

            $month = $_POST['chart-month-select'];

            $add_checkbox = $_POST['all_employees'];
            ...
            if($add_checkbox == 1)
            {
                $all_nassp_employees = "SELECT * FROM nassp_employees";

                $all_nassp_employees1 = mysqli_query($db,$all_nassp_employees);

                $all_nassp_employees2  = mysqli_num_rows($all_nassp_employees1);

                $open = "open";

                $closed = "closed";

                $idnum_employees = rand(10000,99999);

                for($x = 0; $x<$all_nassp_employees2; $x++)
                {
                    $idnum_user = rand(10000,99999);

                   $check_idnum_user="SELECT * from names WHERE id='$idnum_user'";

                   $run_check_idnum_user = mysqli_query($db,$check_idnum_user);

                   $exist_idnum_users = mysqli_num_rows($run_check_idnum_user);

                   if($exist_idnum_users == 0)
                   {
                       $row = mysqli_fetch_array($all_nassp_employees1);
                   ...
                   ('$employee','$name','$month','$idnum_employees','$idnum_user','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open','$open')";
                   ...
                   $_SESSION['page'] = 2;
                       ...
                       echo"<script>alert('Id number is duplicate please try again');</script>";
                   }
                }
        ...
        else if($_SESSION['page'] == 2)
        {
            ...
            {
                $_SESSION['edit_chart'] = 1;

            }

            if($_SESSION['edit_chart'] == 2)
            {
                $table_no = 1;
                ...
                if($_SESSION['table_no'] == 2)
                {
                    echo"<div class='chart-table1' border='5'>";
                echo "<table cellpadding='10' class='chart-table' class='table-danger'>";
                ...
                echo"
                    </tr class='table-danger'><td class='td_regular'>Name</td>
                    <td>Mission</td>
                    <td>Month</td>
                    <td>ID</td>
                    <td>1</td>
                ...
                $row1 = mysqli_fetch_array($run_sel);
                    ...
                    echo"
                    <tr>
                    <td class='td_regular'>".$row1['employee']."</td>
                    <td>".$row1['table1']."</td>
                    <td>".$row1['month1']."</td>
                    <td>".$row1['id']."</td>
                    <td>".$row1['1']."</td>
                ...
                <button name='set_date' class='set_date'>Set Dates as CLOSED</button>

                <button name='set_date_closed' class='set_date_closed'>Set Dates as OPEN</button>
                </form>
                ";

                //code to change table
                ...
                if(isset($_POST['set_date_closed']))
                {
                    ...
                    for($x=0; $x<$names_lenght_row;$x++)
                    {
                        ...
                        for($i=$start_date;$i<=$end_date;$i++)
                        {
                            ...
                            $change = "UPDATE employees SET `$i`='open' WHERE userid='$name_id'";
                            mysqli_query($db,$change);
                ...
                echo "
                    <form method='POST' action=''>
                        <div class='view_names'>
                            <table class='view_names_table'>
                                ...
                                for($x = 0; $x<$row_cell; $x++){
                                    ...
                                    echo"<tr><td class='td_regular'>".$row1['employee']." : ".$row1['userid']."</td></tr>";
                                }
                        ...
                        <div class='insert_name_id'>
                            <input type='text' class='insert_name_text' name='insert_name_text' placeholder='Input staff id'>
                            <button type='submit' class='insert_name_button' name='insert_name_button'>Add employee</button>
                	    </div>
                    </form>        
                ";

            if(isset($_POST['insert_name_button'])  )    
        {
            $username= $_POST['insert_name_text'];
            ...
            if($username == "" )
        {
            ...
            $name_value = $con1b['employee'];

            if($username == $con1c)
            {
                $insert_name_value="INSERT INTO names (names,id) 
                VALUES ('$name_value','$con1c')";
                
                mysqli_query($db,$insert_name_value);
            }
        ...
        echo "
                    <form method='POST' action=''>
                        <div class='show_names'>
                            <table class='show_names_table'>
                                "; 

                                $id_session = $_SESSION['id'];

                                $select_employee="SELECT * FROM names";
                                ...
                                for($x = 0; $x<$row_cell; $x++){
                                    ...
                                    echo"<tr><td class='td_regular'>".$row1['names']." : ".$row1['id']."</td></tr>";
                                }
                        ...
                        <div class='remove_name_id'>
                            <input type='text' class='insert_name_text' name='remove_name_text' placeholder='Input staff id'>
                            <button type='submit' class='insert_name_button' name='remove_name_button'>Remove employee</button>
                	    </div>
                    </form>        
                ";

                if(isset($_POST['remove_name_button'])  ) 
                {
                    $username= $_POST['remove_name_text'];

                    $remove_name = "DELETE FROM names WHERE id = '$username'";

                    mysqli_query($db,$remove_name);
                }
